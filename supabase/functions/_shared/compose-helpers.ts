...
3) "Regarding your request:" section — the core. One short line per service/question they raised, each answering it CONCRETELY with the price/rate/condition from KB ANSWERS, e.g. "- Crew change: in-port coordination fee USD 750 up to 10 pax, USD 950 from 10 pax, USD 95 per crew member." Answers about the estimated disbursement / PDA / total cost -> use the estimated_disbursement_usd figure; about tugs -> the tug count; about port stay / duration -> the estimated port stay; about the berth -> the terminal. NEVER write "we will confirm on nomination" for disbursement, tugs, port stay or terminal when that figure is present in the data — only defer for an item where NO figure exists anywhere. Each on its own line.
...
5) If "estimated_disbursement_usd" is provided: "Based on the above, our estimated disbursement for this call is in the region of USD [amount]." When "cost_breakdown" is also provided, you MAY add ONE natural sentence naming the 2-3 largest components with their figures verbatim (e.g. "the main components being towage USD X, pilotage USD Y and port dues USD Z") — never invent, round or alter a number. Otherwise offer to revert with a full EDA upon confirmation.
